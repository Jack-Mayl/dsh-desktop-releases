...
**本仓库 = 桌面版源码 + 安装包发布**（一个仓库全搞定）。
引擎源码（含插件市场/插件说明等定制）：[Jack-Mayl/deepseek-harness](https://github.com/Jack-Mayl/deepseek-harness)。
...
- 首次启动自动解压内置引擎（约 30–60 秒），之后秒开
- 已装用户自动收到更新提示（启动时 + 每 4 小时），托盘一键安装
- 未签名触发 SmartScreen 时：「更多信息」→「仍要运行」

## 功能总览

| 功能 | 说明 |
|---|---|
| **一键安装** | 205MB 安装包自带引擎 + 便携 Node + pnpm |
| **自动更新** | GitHub Releases 渠道，失败自动重试（60s 退避） |
